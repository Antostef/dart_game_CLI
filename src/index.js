...
const tourdumonde   = require('./tourdumonde.js')
const troiscentun   = require('./troiscentun.js')
...
// TODO: Make a function to shuffle the player group before each new game
...
const around_the_world = async(players, gametype) => {
    ...
    // Initialize
    players = gametype.setup(players)
    ...
    while (!is_won) {
        ...
        while (player_i < players.length && !is_won) {
            ...
            while (shot_i < 3 && !is_won) {
                ...
                players[player_i] = gametype.play(players[player_i], shot_score.SHOT)
...
// TODO: test this gametype to see if it works
// TODO: handle errors in the user input
...
const run = async() => {
    ...
    while(i <= init_questions.NUMBER_OF_PLAYERS) {
